...
module.exports.getList = (req, res) => {
    ...
    Banner.find(query).sort({ createdAt: 'asc' }).then(list => responseOk(res, list))
...
module.exports.getDetail = async (req, res) => {
    ...
    return Banner.findById(id)
        .then(item => {
            if (!item)
                return responseError(res, 404, 'BANNER_NOT_FOUND');
            responseOk(res, item);
        })
        .catch(error => {
...
module.exports.create = async (req, res) => {
    ...
    Banner.init().then(async () => {
        ...
        let { body } = req;
        const banner_id = Math.ceil(Math.random() * 100000);
        ...
        if (banner)
            return responseError(res, 409, 'BANNER_ID_IS_EXISTED');

        const collection = new Banner({
            ...body,
            banner_id,
...
module.exports.edit = (req, res) => {
    Banner.init().then(async () => {
        ...
        const banner = await Banner.findOne({ banner_id: req.params.banner_id });
...
module.exports.remove = (req, res) => {
    return Banner.init().then(async () => {
        ...
        // await Question.deleteMany({ banner_id });
