...
class GamePage extends Component {
  constructor(props) {
    ...
    socketGlobal.on("responseQuestion", question => {
      if (question.success) {
        this.setState({
          ...
          peopleSelectD: 0,
          totalSelect: 0,
          checkFinished: question.data.lastQuestion
        });
    ...
    socketGlobal.on("responseResult", result => {
      ...
      const { ordinalNumber, rightAnswer, explain } = result.data;
      if (ordinalNumber === this.state.question.ordinalNumber) {
        ...
        if (rightAnswer === this.state.answer) {
          ...
          this.props.handleUserLost(true);
        }
        ...
        <div className="popuper">
          <div className="popup-container">
            <div className="popup-content">
              {checkFinished
                ? "Game đã kết thúc! Hẹn gặp lại!"
                : "Bạn có muốn tiếp tục tham gia với tư cách khán giả không?"}
            </div>
            <div className="popup_bottom justify-content-center">
              <Button hidden={checkFinished} onClick={this.handleViewDemo}>
                Tiếp tục
              </Button>
              <Button onClick={this.handleOnBack}>Rời đi</Button>
            </div>
          </div>
        </div>
      )
